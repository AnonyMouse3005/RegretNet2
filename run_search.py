...
def model_builders(model_classes: List[Type[pl.LightningModule]], n: int, k: int, d: int):
    builders = [
        ...
        functools.partial(RegretNetSystem, n=n, k=k, lr=0.005, gamma=0.99, hidden_layer_channels=[40, 40, 40, 40]),
    ]
...
def worker_func(model_builder, data_module_builder, num_epoch, seed) -> Tuple[Record, Record]:
    ...
    with contextlib.redirect_stdout(open(os.devnull, 'w')):
        with contextlib.redirect_stderr(open(os.devnull, 'w')):
            ...
            model = model_builder()
            train_record, test_record = train_and_test_once(model, data_module, num_epoch, device_id)
...
def main():
    ...
    model_classes = [NonSPRuleSystem, PercentileRuleSystem, MoulinNetSystem, RegretNetSystem]
    num_trials = [1, 1, 4, 10]
    num_epochs = [1, 3, 100, 3000]
    ...
    with mp.Pool(processes=len(devices), initializer=worker_init, initargs=(device_id_queue, )) as p:
            ...
            for d in d_range:
                ...
                for k in k_range:
                    for model_builder, num_trial, num_epoch in zip(
                            model_builders(model_classes, n, k, d),
                            num_trials,
                            num_epochs
                    ):
        ...
        for r in results:
            ...
            train_record, test_record = min(record_list, key=lambda r: r[1].metric)
            print(f'Best record: {test_record}')
            train_record.write_csv(tag)
            test_record.write_csv(tag)
